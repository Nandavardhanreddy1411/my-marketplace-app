...
function Register() {
  ...
  const formik = useFormik({
    initialValues: { firstName:'', lastName:'', email:'', password:'', confirmPassword:'', role:'Customer', terms:false },
  ...
  const labelStyle = { fontWeight:'600', marginBottom:'8px', display:'block', color:'#333', marginTop:'15px' };
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg, #4F46E5, #06B6D4)', display:'flex', alignItems:'center', justifyContent:'center', padding:'40px 20px' }}>
      <SEO title="Register" description="Create your Pixer account and start buying or selling digital products." />
      <div style={{ background:'white', borderRadius:'20px', padding:'50px 40px', width:'100%', maxWidth:'540px', boxShadow:'0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'15px' }}>
            <div>
              ...
              {formik.touched.firstName && formik.errors.firstName && <p style={errorStyle}>{formik.errors.firstName}</p>}
            </div>
            <div>
              ...
              {formik.touched.lastName && formik.errors.lastName && <p style={errorStyle}>{formik.errors.lastName}</p>}
            </div>
          </div>
          ...
          {formik.touched.confirmPassword && formik.errors.confirmPassword && <p style={errorStyle}>{formik.errors.confirmPassword}</p>}

          <label style={labelStyle}>Join as</label>
          <select name="role" onChange={formik.handleChange} value={formik.values.role}
            style={{ width:'100%', padding:'14px', marginBottom:'15px', border:'1.5px solid #e2e8f0', borderRadius:'10px', fontFamily:'Poppins', fontSize:'0.95rem', outline:'none', background:'#f8fafc', color:'#1e1e2f' }}>
          ...
          <label style={{ fontSize:'0.9rem', color:'#555', display:'flex', alignItems:'center', gap:'8px', marginBottom:'5px' }}>
            ...
            I agree to the{' '}
            <Link to="/terms" style={{ color:'#4F46E5', textDecoration:'none', fontWeight:'600' }}>
              Terms & Conditions
            </Link>
          </label>
          {formik.touched.terms && formik.errors.terms && <p style={{ ...errorStyle, marginBottom:'15px' }}>{formik.errors.terms}</p>}

          <button type="submit" style={{ width:'100%', padding:'14px', background:'#4F46E5', color:'white', border:'none', borderRadius:'10px', fontSize:'1rem', fontWeight:'600', cursor:'pointer', fontFamily:'Poppins', marginTop:'15px', marginBottom:'20px' }}>
            Create Account
          </button>
