...
function PreviewModal({ product, onClose, onAddToCart, onNavigate }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={onClose} style={{
          ...
          borderRadius:'50%', fontSize:'1.1rem', cursor:'pointer', fontWeight:'700'
        }}>✕</button>

        <div style={{marginBottom:'20px'}}>
          <span style={{background:'#EEF2FF', color:'#4F46E5', padding:'4px 12px', borderRadius:'20px', fontSize:'0.8rem', fontWeight:'500'}}>
            {product.category}
          </span>
          <h3 style={{color:'#1e1e2f', fontWeight:'700', margin:'10px 0 5px'}}>{product.title}</h3>
        ...
        <div style={{display:'flex', gap:'15px', marginBottom:'20px', flexWrap:'wrap'}}>
          <div style={{flex:1, background:'#F8FAFC', borderRadius:'12px', padding:'15px', minWidth:'150px'}}>
            <p style={{margin:0, color:'#888', fontSize:'0.8rem'}}>Sale Price</p>
            <h3 style={{margin:0, color:'#4F46E5', fontWeight:'700'}}>₹{product.price.toLocaleString('en-IN')}</h3>
          </div>
          <div style={{flex:1, background:'#F8FAFC', borderRadius:'12px', padding:'15px', minWidth:'150px'}}>
...
function Products() {
  ...
  return (
    <div style={{background:'#F8FAFC', minHeight:'100vh', paddingBottom:'60px'}}>
      <SEO title="Products" description="Browse 9+ premium digital products on Pixer Marketplace." keywords="buy templates, UI kits, source code, react components" />
      ...
      <div style={{background:'linear-gradient(135deg, #4F46E5, #06B6D4)', padding:'60px 0 40px', textAlign:'center', color:'white'}}>
        <h1 style={{fontSize:'2.5rem', fontWeight:'700', marginBottom:'10px'}}>🛍️ Product Marketplace</h1>
        <p style={{fontSize:'1.1rem', opacity:0.9}}>Browse {allProducts.length}+ premium digital products</p>
        <div style={{maxWidth:'500px', margin:'25px auto 0', display:'flex', gap:'10px', padding:'0 20px'}}>
          <input type="text" placeholder="🔍 Search products..."
            onChange={(e) => setSearch(e.target.value)}
            style={{flex:1, padding:'14px 20px', borderRadius:'25px', border:'none', fontSize:'1rem', fontFamily:'Poppins', outline:'none', color:'#'}} />
          <button style={{background:'#F59E0B', color:'white', border:'none', padding:'14px 25px', borderRadius:'25px', fontWeight:'600', cursor:'pointer', fontFamily:'Poppins'}}>Search</button>
        </div>
      </div>
      ...
      <div className="container-fluid px-5" style={{marginTop:'40px'}}>
        <div className="row">
          ...
          <div className="col-md-3">
            <div style={{background:'white', borderRadius:'16px', padding:'25px', boxShadow:'0 4px 15px rgba(0,0,0,0.07)', position:'sticky', top:'80px'}}>
                ...
                {categories.map(cat => (
                  <div key={cat} style={{marginBottom:'8px'}}>
                    <label style={{display:'flex', alignItems:'center', gap:'8px', cursor:'pointer', fontSize:'0.9rem', color:'#444'}}>
                      <input type="radio" name="category" checked={selectedCategory === cat} onChange={() => setSelectedCategory(cat)} style={{accentColor:'#4F46E5'}} />
                      {cat}
                    </label>
                  </div>
                ))}
              ...
              <div style={{marginBottom:'25px'}}>
                ...
                <input type="range" min="500" max="6000" value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} style={{width:'100%', accentColor:'#4F46E5'}} />
              ...
              <div style={{marginBottom:'25px'}}>
                <h6 style={{color:'#4F46E5', fontWeight:'600', marginBottom:'12px'}}>Min Rating</h6>
                {[0, 3, 3.5, 4, 4.5].map(r => (
                  <div key={r} style={{marginBottom:'8px'}}>
                    <label style={{display:'flex', alignItems:'center', gap:'8px', cursor:'pointer', fontSize:'0.9rem', color:'#444'}}>
                      <input type="radio" name="rating" checked={minRating === r} onChange={() => setMinRating(r)} style={{accentColor:'#4F46E5'}} />
              ...
              <div style={{marginBottom:'25px'}}>
                <h6 style={{color:'#4F46E5', fontWeight:'600', marginBottom:'12px'}}>Vendor Type</h6>
                {['All', 'Individual', 'Agency'].map(type => (
                  <div key={type} style={{marginBottom:'8px'}}>
                    <label style={{display:'flex', alignItems:'center', gap:'8px', cursor:'pointer', fontSize:'0.9rem', color:'#444'}}>
                      <input type="radio" name="vendorType" checked={vendorType === type} onChange={() => setVendorType(type)} style={{accentColor:'#4F46E5'}} />
              ...
              <div style={{marginBottom:'25px'}}>
                ...
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} style={{width:'100%', padding:'10px', border:'1.5px solid #e2e8f0', borderRadius:'10px', fontFamily:'Poppins', fontSize:'0.9rem', outline:'none', color:'#1e1e2f', background:'#f8fafc'}}>
              ...
              <button onClick={() => { setSearch(''); setSelectedCategory('All'); setSortBy('default'); setMaxPrice(6000); setMinRating(0); setVendorType('All'); }}
                style={{width:'100%', padding:'10px', background:'#ef4444', color:'white', border:'none', borderRadius:'10px', fontWeight:'600', cursor:'pointer', fontFamily:'Poppins', fontSize:'0.9rem'}}>
                🔄 Reset Filters
              </button>
            </div>
          </div>
          ...
          <div className="col-md-9">
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px', flexWrap:'wrap', gap:'10px'}}>
              <p style={{color:'#666', margin:0}}>Showing <strong style={{color:'#4F46E5'}}>{filtered.length}</strong> products</p>
              {search && <p style={{color:'#666', margin:0}}>Results for: <strong style={{color:'#4F46E5'}}>"{search}"</strong></p>}
            </div>
            ...
            <div className="row g-4">
              {filtered.map(product => (
                <div className="col-md-4" key={product.id}>
                  <div style={{background:'white', borderRadius:'16px', overflow:'hidden', boxShadow:'0 4px 15px rgba(0,0,0,0.07)', transition:'all 0.3s ease', height:'100%'}}
                    ...
                    <div style={{position:'relative'}}>
                      ...
                      {product.badge && (
                        <span style={{
                          ...
                          background: product.badge === 'Bestseller' ? '#F59E0B' : product.badge === 'Hot' ? '#ef4444' : product.badge === 'New' ? '#22c55e' : '#4F46E5',
                          color:'white', padding:'4px 10px', borderRadius:'20px', fontSize:'0.75rem', fontWeight:'600'
                        }}>{product.badge}</span>
                      )}
                      <div style={{position:'absolute', inset:0, background:'rgba(79,70,229,0.7)', display:'flex', alignItems:'center', justifyContent:'center', opacity:0, transition:'opacity 0.3s ease'}}
                        onMouseEnter={e => e.currentTarget.style.opacity='1'}
                        onMouseLeave={e => e.currentTarget.style.opacity='0'}
                        onClick={() => setPreviewProduct(product)}>
                        <button style={{background:'white', color:'#4F46E5', border:'none', padding:'10px 22px', borderRadius:'25px', fontWeight:'700', cursor:'pointer', fontSize:'0.9rem', fontFamily:'Poppins'}}>
                          👁️ Quick Preview
                        </button>
                      </div>
                    </div>
                    ...
                    <div style={{padding:'20px'}}>
                      <div style={{display:'flex', gap:'6px', marginBottom:'8px', flexWrap:'wrap'}}>
                        <span style={{background:'#EEF2FF', color:'#4F46E5', padding:'3px 10px', borderRadius:'20px', fontSize:'0.72rem', fontWeight:'500'}}>{product.category}</span>
                        <span style={{background: product.vendorType === 'Agency' ? '#FEF3C7' : '#f0fdf4', color: product.vendorType === 'Agency' ? '#F59E0B' : '#16a34a', padding:'3px 10px', borderRadius:'20px', fontSize:'0.72rem', fontWeight:'500'}}>
                          {product.vendorType === 'Agency' ? '🏢' : '👤'} {product.vendorType}
                        </span>
                      </div>
                      <h5 style={{color:'#1e1e2f', fontWeight:'700', margin:'0 0 5px', fontSize:'0.95rem'}}>{product.title}</h5>
                      <p style={{color:'#888', fontSize:'0.8rem', margin:'0 0 8px'}}>By {product.vendor}</p>
                      ...
                      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:'8px'}}>
                        ...
                        <div style={{display:'flex', gap:'6px'}}>
                          <button onClick={() => setPreviewProduct(product)} style={{background:'#F8FAFC', color:'#4F46E5', border:'1.5px solid #e2e8f0', padding:'7px 10px', borderRadius:'20px', fontWeight:'600', cursor:'pointer', fontSize:'0.75rem', fontFamily:'Poppins'}}>👁️</button>
                          <button onClick={() => navigate(`/product-details/${product.id}`)} style={{background:'transparent', color:'#4F46E5', border:'1.5px solid #4F46E5', padding:'7px 10px', borderRadius:'20px', fontWeight:'600', cursor:'pointer', fontSize:'0.75rem', fontFamily:'Poppins'}}>Details</button>
                          {/* <button onClick={() => handleAddToCart(product)} style={{background:'#4F46E5', color:'white', border:'none', padding:'7px 12px', borderRadius:'20px', fontWeight:'600', cursor:'pointer', fontSize:'0.75rem', fontFamily:'Poppins'}}>Buy Now</button> */}
                          <button
